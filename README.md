...
After joining, use `openclaw awn peers` or `awn_list_peers()` to see the co-members that are now reachable.
...
| `awn_list_peers` | List AWN peers reachable in joined worlds, optionally filtered by capability |
...
| `awn_list_peers` is empty | Expected until you join a world |
